(()=>{
  ...
  function normalizeState(){
    if(!Number.isFinite(state.pieces))state.pieces=0;
    state.pieces=Math.max(0,Math.floor(state.pieces));
    state.world=Math.floor(state.pieces/PIECES_PER_PUZZLE);
    if(!state.randomPieceSlots || typeof state.randomPieceSlots!=="object")state.randomPieceSlots={};
    const currentCount=state.pieces%PIECES_PER_PUZZLE;
    const key=String(state.world);
    let slots=Array.isArray(state.randomPieceSlots[key])?state.randomPieceSlots[key].filter(n=>Number.isInteger(n)&&n>=0&&n<PIECES_PER_PUZZLE):[];
    slots=[...new Set(slots)];
    const available=()=>Array.from({length:PIECES_PER_PUZZLE},(_,i)=>i).filter(i=>!slots.includes(i));
    while(slots.length<currentCount){
      const a=available(); if(!a.length)break;
      slots.push(a[Math.floor(Math.random()*a.length)]);
    }
    if(slots.length>currentCount)slots=slots.slice(0,currentCount);
    state.randomPieceSlots[key]=slots;
    save();
  }

  function slotsFor(world,count){
    if(count>=PIECES_PER_PUZZLE)return Array.from({length:PIECES_PER_PUZZLE},(_,i)=>i);
    const key=String(world);
    if(!state.randomPieceSlots[key])state.randomPieceSlots[key]=[];
    let slots=[...new Set(state.randomPieceSlots[key])].filter(n=>Number.isInteger(n)&&n>=0&&n<PIECES_PER_PUZZLE);
    while(slots.length<count){
      const rest=Array.from({length:PIECES_PER_PUZZLE},(_,i)=>i).filter(i=>!slots.includes(i));
      if(!rest.length)break;
      slots.push(rest[Math.floor(Math.random()*rest.length)]);
    }
    state.randomPieceSlots[key]=slots.slice(0,count);
    return state.randomPieceSlots[key];
  }

  function awardRandomPiece(){
    normalizeState();
    const world=Math.floor(state.pieces/PIECES_PER_PUZZLE);
    const count=state.pieces%PIECES_PER_PUZZLE;
    const current=slotsFor(world,count);
    const rest=Array.from({length:PIECES_PER_PUZZLE},(_,i)=>i).filter(i=>!current.includes(i));
    const won=rest[Math.floor(Math.random()*rest.length)];
    current.push(won);
    state.randomPieceSlots[String(world)]=current;
    state.pieces+=1;
    state.world=Math.floor(state.pieces/PIECES_PER_PUZZLE);
    if(state.pieces%PIECES_PER_PUZZLE===0 && !state.randomPieceSlots[String(state.world)])state.randomPieceSlots[String(state.world)]=[];
    save();
    return {world,index:won,completed:current.length===PIECES_PER_PUZZLE};
  }
  ...
  function addPuzzleStyles(){
    ...
    const s=document.createElement('style');s.id='fivePuzzleStyle';s.textContent=`
      .jigsaw,.miniPuzzle,.earnedMiniPuzzle,.galleryThumb{display:grid!important;grid-template-columns:repeat(4,1fr)!important;grid-template-rows:repeat(4,1fr)!important;background:#dfe7e2!important;overflow:hidden}
      ...
      .piece,.miniPiece,.earnedMiniPiece,.galleryPiece{position:relative;background-repeat:no-repeat;opacity:.10;filter:grayscale(1);border:1px solid #ffffffaa;transition:opacity .25s,filter .25s,transform .25s}
      .piece.on,.miniPiece.on,.earnedMiniPiece.on,.galleryPiece.on{opacity:1;filter:none}
      ...
      .piece.newPiece,.miniPiece.newPiece,.earnedMiniPiece.newPiece{animation:piecePop .9s ease}
      @keyframes piecePop{0%{transform:scale(.45) rotate(-8deg);opacity:.1}55%{transform:scale(1.14) rotate(2deg);opacity:1}100%{transform:scale(1)}}
      ...
      .pieceHole{background-image:none!important;background:#dfe6e2!important;opacity:.62!important;filter:none!important}
    `;document.head.appendChild(s);
  }

  function fillGrid(box,world,unlocked,newIndex=-1,kind='piece'){
    if(!box)return;
    box.innerHTML='';
    ...
    for(let i=0;i<PIECES_PER_PUZZLE;i++){
      ...
      const on=unlockedSet.has(i);
      p.className=`${kind}${on?' on':' pieceHole'}${i===newIndex?' newPiece':''}`;
      if(on){
        p.style.backgroundImage=bg;
        p.style.backgroundSize='400% 400%';
        const col=i%GRID,row=Math.floor(i/GRID);
        p.style.backgroundPosition=`${(col/(GRID-1))*100}% ${(row/(GRID-1))*100}%`;
      }
      box.appendChild(p);
    }
  }
  ...
  function currentInfo(){
    normalizeState();
    return {world:Math.floor(state.pieces/PIECES_PER_PUZZLE),count:state.pieces%PIECES_PER_PUZZLE};
  }

  renderPuzzleGallery=function(){
    const gallery=$('#puzzleGallery'); if(!gallery)return;
    gallery.innerHTML='';
    const completed=Math.floor(state.pieces/PIECES_PER_PUZZLE);
    ...
    for(let w=0;w<completed;w++){
      ...
      const thumb=document.createElement('div');thumb.className='galleryThumb';
      fillGrid(thumb,w,Array.from({length:PIECES_PER_PUZZLE},(_,i)=>i),-1,'galleryPiece');
      const meta=document.createElement('div');meta.className='galleryMeta';
      meta.innerHTML=`<div class="galleryName">${themes[w%themes.length]}・${w+1}</div><div class="galleryBadge">完成！</div>`;
      item.appendChild(thumb);item.appendChild(meta);gallery.appendChild(item);
    }
  };

  puzzle=function(newIndex=-1,displayWorld=null){
    addPuzzleStyles();normalizeState();
    const info=currentInfo();
    const world=displayWorld===null?info.world:displayWorld;
    const count=world<info.world?PIECES_PER_PUZZLE:(world===info.world?info.count:0);
    const unlocked=world<info.world?Array.from({length:PIECES_PER_PUZZLE},(_,i)=>i):slotsFor(world,count);
    ...
    fillGrid($('#jigsaw'),world,unlocked,newIndex,'piece');
    fillGrid($('#miniPuzzle'),info.world,slotsFor(info.world,info.count),-1,'miniPiece');
    $('#puzzleStatus').textContent=`この絵は ${count} / ${PIECES_PER_PUZZLE} ピース。もらえる場所は毎回ランダム！`;
    renderPuzzleGallery();
  };

  finish=function(){
    const award=awardRandomPiece();
    $('#scoreText').textContent=`${score} / ${TOTAL} 問 せいかい`;
    $('#pieceEarned').textContent=award.completed?'🎉 1ピース GET！ 4×4パズル完成！':'🧩 1ピース GET！ ランダムな場所にはまりました！';
    puzzle(award.completed?-1:award.index,award.world);
    show('result');
    ...
    if(!card){
      ...
      $('#pieceEarned').insertAdjacentElement('afterend',card);
    }
    const unlocked=award.completed?Array.from({length:PIECES_PER_PUZZLE},(_,i)=>i):slotsFor(award.world,(state.pieces-1)%PIECES_PER_PUZZLE+1);
    card.innerHTML='<b>今回のピースがはまったよ！</b><div id="earnedMiniPuzzle" class="earnedMiniPuzzle"></div><small>色が見えているところが、集めたピースです。</small>';
    fillGrid($('#earnedMiniPuzzle'),award.world,unlocked,award.completed?-1:award.index,'earnedMiniPiece');
  };
  ...
  function patchLabels(){
    ...
    const qc=$('#qCount');if(qc)qc.textContent='1 / 5';
    puzzle();
